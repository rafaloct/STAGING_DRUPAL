...
SQL, $params);
  ...
  /**
   * Syncs a Drupal node to BigQuery audit tables.
   */
  public function syncNodeToBigQuery(\Drupal\node\NodeInterface $node): void {
    try {
      $client = $this->bigQueryClient();
      if (!$client) {
        $this->logger->warning('BigQuery client not configured for sync');
        return;
      }
      ...
      $query = <<<SQL
        ...
        ON CONFLICT (hash_auditoria) DO UPDATE SET
          data_referencia = CURRENT_TIMESTAMP(),
      ...
      $this->runQuery($client, $query, [
        ...
        'table_name' => $bundle,
        'record_id' => $node->id(),
        'content' => substr($title, 0, 1000),
        'sigilo' => 'publico',
        'hash' => hash('sha256', 'drupal_' . $node->id() . $node->getChangedTime()),
        'uri' => \Drupal::request()->getSchemeAndHttpHost() . $node->toUrl()->toString(),
        'article' => '',
      ]);

      $this->logger->info('Synced node @id to BigQuery', ['@id' => $node->id()]);
    }
    catch (\Throwable $exception) {
      $this->logger->error('Failed to sync node @id: @error', [
        '@id' => $node->id(),
  ...
  /**
   * Marks a node as deleted in BigQuery.
   */
  public function markNodeDeletedInBigQuery(int $nodeId): void {
    try {
      $client = $this->bigQueryClient();
      if (!$client) {
        return;
      ...
      $query = <<<SQL
        DELETE FROM `$table`
        WHERE id_citacao = @id OR id_registro_original = @record_id
      SQL;
      ...
      $this->logger->info('Marked node @id as deleted in BigQuery', ['@id' => $nodeId]);
    }
    catch (\Throwable $exception) {
      $this->logger->error('Failed to mark node @id as deleted: @error', [
        '@id' => $nodeId,
        '@error' => $exception->getMessage(),
      ]);
    }
